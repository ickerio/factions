...
public class InviteCommand implements Command {
    private int list(CommandContext<ServerCommandSource> context) throws CommandSyntaxException {
        ...
        new Message(Text.translatable("factions.command.invite.list",
    ...
    private int add(CommandContext<ServerCommandSource> context) throws CommandSyntaxException {
        ...
        if (faction.isInvited(player.getUuid())) {
            new Message(
                    Text.translatable("factions.command.invite.add.fail.already_invited", target.getName().getString()))
        ...
        if (faction.getID().equals(targetFaction)) {
            new Message(
                    Text.translatable("factions.command.invite.add.fail.already_member", target.getName().getString()))
                    .fail().send(player, false);
            return 0;
        }
