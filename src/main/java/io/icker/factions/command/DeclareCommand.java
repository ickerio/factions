...
public class DeclareCommand implements Command {
    ...
    private int updateRelationship(CommandContext<ServerCommandSource> context,
        ...
        if (rel.status == rev.status) {
            ...
            new Message(
                    Text.translatable("factions.command.declare.success.mutual", msgStatus, targetFaction.getName()))
                    .send(sourceFaction);
            new Message(
        ...
        new Message(Text.translatable("factions.command.declare.success.actor", targetFaction.getName(), msgStatus))
                .send(sourceFaction);
